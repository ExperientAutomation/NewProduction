...
//		System.out.println("Status change text: "+statuschange1.getFirstSelectedOption().getText()+"lenth:"+ statuschange1.getAllSelectedOptions().size());
			...
			wait.until(ExpectedConditions
					.visibilityOfAllElementsLocatedBy(By.xpath("//*[contains(.,'#Print items this run ')]/parent::table")));
			int quickcount = driver.findElements(By.xpath("//*[contains(.,'#Print items this run ')]/parent::table")).size();
			...
			if (!links1.get(i).getAttribute("class").equalsIgnoreCase("disabled")) {
				links1.get(i).click();
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//button[contains(.,'Export')]")));
			int exporttable = driver.findElements(By.xpath("//table[@class='DataGrid autowidth']")).size();
